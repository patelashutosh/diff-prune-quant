...
class Diffusion(object):
    def __init__(self, args, config, config_unet, device=None):
        self.args = args
        self.config = config
    ...
    def sample(self):
        ...
        if self.config.data.dataset == "CIFAR10":
            ...
            raise ValueError
        if(self.args.use_local_model):
            ckpt = self.args.local_model
        else:
            ckpt = get_ckpt_path(f"ema_{name}")
        logger.info("Loading checkpoint {}".format(ckpt))
        unet = torch.load(ckpt, map_location=self.device)
        subfolder = 'unet' if os.path.isdir(os.path.join(ckpt, 'unet')) else None
        # model = UNet2DModel.from_pretrained(ckpt, subfolder=subfolder)
...
if __name__ == "__main__":
    ...
    with open(args.config_unet) as f:
        config_unet = json.loads(f.read())
    ...
    runner = Diffusion(args, config, config_unet)
    runner.sample()
